...
static void ColouriseDoc(unsigned int startPos, int length, int initStyle,
	...
	for (; sc.More(); sc.Forward()) {
		...
		if (sc.atLineStart) {
			if (sc.state == SCE_D_CHARACTER) {
				//MOVE: line continuation appears only in char literal.
				//REMOVE: Do we even need this code? This case is improbable.
				// Only char literal is not multiline in D.
		...
		switch (sc.state) {
			...
			case SCE_D_NUMBER:
				...
				else if ( ( sc.ch == '-' || sc.ch == '+' ) && numFloat && (
					( numHex && ( sc.chPrev == 'p' || sc.chPrev == 'P' ) ) ||
					( sc.chPrev == 'e' || sc.chPrev == 'E' ) ) ) {
					// Parse exponent sign in float literals
					continue;
				}
				else {
					sc.SetState(SCE_D_DEFAULT);
